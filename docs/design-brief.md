...
The controls are Tabletop Simulator's, because that is the vocabulary the audience for this already has: left-drag moves a card, left-click taps it, right-click opens everything else it can do, F turns it over, Q and E rotate, G stacks a selection together, Delete removes tokens, the number row draws that many cards, R shuffles, and Alt reads whatever is under the cursor. Turns pass whole rather than a step at a time - Enter ends yours - because a phase marker advanced by hand is bookkeeping in a game whose rules already live in the players' heads. A control scheme this borrowed is also completely undiscoverable, so F1 lists all of it and the bar along the bottom carries the handful needed in the first minute.
