...
3. The decks in your inventory are listed. Choose one and it goes down shuffled, or borrow one where the server lends them.

A deck that is not legal in the chosen format is not refused. You are shown what is not legal and asked whether to use it anyway; if you do, everybody at the table is told, and so is anybody who comes to it later.

Right-click the table while a game is on to sit at the board, and to choose a deck if yours is not down. Between games of a match, the same click lets you sideboard, or starts the next game.

## What the board tells you

The row along the top shows each player's face, life, hand and library, and whose turn it is. Beside the turn it says what the table is playing, with anything unusual - played for keeps, no format - in amber. Rest the pointer there to read all of it.

Nothing here checks whether a play is legal. That is for the people at the table.
