 ...
 * @param podSeed which pod it came out of, so two pools from two drafts are never confused -
 *                and so a deck check can say which draft it is checking against
 */
public record DraftedPool(List<CardComponent> cards, String podSeed) {

    public DraftedPool {
        cards = cards == null ? List.of() : List.copyOf(cards);
        podSeed = podSeed == null ? "" : podSeed;
    }
    ...
    public static final Codec<DraftedPool> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.STRING.optionalFieldOf("pod", "").forGetter(DraftedPool::podSeed))
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, DraftedPool> STREAM_CODEC =
            StreamCodec.composite(
                    ...
                    DraftedPool::cards,
                    ByteBufCodecs.stringUtf8(64), DraftedPool::podSeed,
                    DraftedPool::new);
