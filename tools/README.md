...
| `mixincheck.py` | Every mixin the mod has written is named in its loader's `gathering.mixins.json`, and every name in one has a class behind it. A mixin nothing applies compiles, ships and does nothing: the hook that sweeps a deck over cards was dropped from both lists in a merge, and the gesture was dead in a build that looked finished. |
